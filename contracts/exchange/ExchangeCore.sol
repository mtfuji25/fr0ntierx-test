...
contract ExchangeCore is ReentrancyGuarded, StaticCaller, EIP712 {
    ...
    function atomicMatch(
        ...
        uint256 sellerValue = _chargePlatformFee(
            firstOrder,
            firstCall,
            secondOrder,
            secondCall
        ...
        if (sellerValue > 0) {
            // sellerValue: the amount of ETH sent via the tx after deducting the platform fee
        ...
        if (secondCall.target != address(0)) {
            // to support purchasing with ETH, skip the second call if the target is 0x0
            require(
                executeCall(
    ...
    function _chargePlatformFee(
        ...
        Order memory secondOrder,
        Call memory secondCall
    ) internal virtual returns (uint256 sellerValue) {
